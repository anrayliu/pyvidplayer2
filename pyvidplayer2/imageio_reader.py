...
class IIOReader:
    ...
    def _probe(self):
        ...
        try:
            p = subprocess.Popen(f"ffprobe -i {"-" if self._as_bytes else self._path} -show_streams -count_frames -select_streams v -loglevel quiet -print_format json", stdin=subprocess.PIPE if self._as_bytes else None, stdout=subprocess.PIPE)
