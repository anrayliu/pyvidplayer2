...
class VideoPyglet(Video):
    """Video playback class for Pyglet."""
    ...
    def _create_frame(self, data):
        return pyglet.image.ImageData(self.current_size[0],
                                      self.current_size[1],
                                      self._vid._colour_format,
                                      np.flip(data, 0).tobytes())
    ...
    def preview(self, max_fps: int = 60) -> None:
        ...
        win = pyglet.window.Window(width=self.current_size[0],
                                   height=self.current_size[1],
